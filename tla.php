...
function tla_ads() {
	...
	$LOCAL_XML_FILENAME = "local_73397.xml";
	...
	if( filemtime($LOCAL_XML_FILENAME) < (time() - 3600) || filesize($LOCAL_XML_FILENAME) < 20) {
		...
		tla_updateLocalXML("http://www.text-link-ads.com/xml.php?inventory_key=UEF9DVZE6VSYR3PFLVSA&referer=" . urlencode($request_uri) .  "&user_agent=" . urlencode($user_agent), $LOCAL_XML_FILENAME, $CONNECTION_TIMEOUT);
	}
	...
	if ( is_array($arr_xml) ) {
		...
		*/
		...
		for ($i = 0; $i < count($arr_xml['URL']); $i++) {
			...
			echo $arr_xml['BeforeText'][$i]." <a href=\"".$arr_xml['URL'][$i]."\">".$arr_xml['Text'][$i]."</a> ".$arr_xml['AfterText'][$i] . ' ';
		}
	}
